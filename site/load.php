...
while ($myrow = $result->fetch_array(MYSQLI_ASSOC)){
    echo "<tr>";
    printf ("<td>%d</td> <td>%s</td> <td>%s</td> <td>%s</td><td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td><td>%d</td> <td>%s</td> <td>%s</td>\n", $myrow["id"], $myrow["type"], $myrow["name"], $myrow["code"], $myrow["storage"], $myrow["place"], $myrow["factor"], $myrow["RFID"], $myrow["count"], $myrow["project"], $myrow["data"] );
    echo "</tr>";
...
<form action="" id="find_components" method="POST" onsubmit="findLabel();return false;">
  ...
  <center>
    ...
    <br>
    <a href="loadResistor.php" class="button">Показать все резисторы</a>
    <a href="loadСapacitor.php" class="button">Показать все конденсаторы</a>
    <br>
    <a href="index.html"class="button">На главную</a>
